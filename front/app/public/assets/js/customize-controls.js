/* global mokaBaseBgColors, mokaBaseColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( mokaBaseBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( mokaBaseBgColors[ context ].color ) {
						backgroundColorValue = mokaBaseBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( mokaBaseBgColors[ context ].setting ).get();
					}
					mokaBaseSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( mokaBaseBgColors ).forEach( function( context ) {
			wp.customize( mokaBaseBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					mokaBaseSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function mokaBaseSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = mokaBaseColor( backgroundColor, accentHue );
