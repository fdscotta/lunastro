/* global mokaBaseBgColors, mokaBasePreviewEls, jQuery, _, wp */
/**
...
( function( $, api, _ ) {
		...
		value.bind( function() {
			...
			setTimeout( function() {
				Object.keys( mokaBaseBgColors ).forEach( function( context ) {
					mokaBaseGenerateColorA11yPreviewStyles( context );
				} );
			}, 50 );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function mokaBaseGenerateColorA11yPreviewStyles( context ) {
		...
		if ( ! _.isUndefined( a11yColors[ context ] ) ) {
			...
			if ( mokaBasePreviewEls[ context ] ) {
				_.each( mokaBasePreviewEls[ context ], function( items, setting ) {
	...
	$( document ).ready( function() {
		mokaBaseGenerateColorA11yPreviewStyles( 'content' );
		mokaBaseGenerateColorA11yPreviewStyles( 'header-footer' );
	} );
